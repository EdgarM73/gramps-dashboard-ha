// Todestage Card analog zur Geburtstagskarte
...
class GrampsTodestageDashboardEditor extends HTMLElement {
    ...
    // Removed the old _fireConfigChanged method as it was duplicated
    // Keeping the customElements.define call intact
}
...
customElements.define('gramps-todestage-dashboard-editor', GrampsTodestageDashboardEditor);
  ...
  render() {
    ...
    this._populatePersonSelector();
    // Entities will be rendered by _updateEntityList()
  }
...
window.customCards.push({
  ...
  description: 'Ein anpassbares Todestage-Dashboard-Template fuer Home Assistant',
  preview: true,
...
class GrampsTodestageDashboardCard extends HTMLElement {
  ...
  render() {
    ...
    if (!nameEntity) {
      console.warn(`Gramps Todestage Dashboard: name_entity fehlt in der Konfiguration`);
      return null;
    }
    ...
    if (!nameEntity_obj) {
      console.warn(`Gramps Todestage Dashboard: Namens-Entitaet ${nameEntity} nicht gefunden`);
      return null;
    }
    ...
    button.innerHTML = `
      <div class="person-image">
        ${imageUrl ? `<img src="${imageUrl}" alt="${name}" />` : `<ha-icon icon="mdi:account"></ha-icon>`}
      </div>
